...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    getAuthConfig()
      .then((result) => {
        if (active) setTestMode(result.test_mode);
      })
      .catch(() => undefined);
    setNow(Date.now());
  ...
  function identity(): LoginIdentity | null {
    const valid =
      channel === "phone" ? /^1[3-9][0-9]{9}$/.test(normalized) : /^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(normalized);
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      window.location.assign("/dashboard/account-center");
    } catch (failure) {
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <TabsList className="w-full" aria-label="注册登录方式">
          <TabsTrigger value="phone" disabled={!!busy}>
            手机号
          </TabsTrigger>
      ...
      <form noValidate onSubmit={onSubmit} className="flex flex-col gap-4">
        {testMode && (
          <Alert>
            <AlertDescription>当前为测试环境：请先获取验证码，再填写 123456。不会实际发送短信或邮件。</AlertDescription>
          </Alert>
        )}
        <FieldGroup className="gap-4">
          <Field data-invalid={!!contactError}>
            <FieldLabel htmlFor="login-contact">
              {channel === "phone" ? "手机号（中国大陆 +86）" : "邮箱地址"}
            </FieldLabel>
            <Input
              id="login-contact"
              value={contact}
              ...
              maxLength={channel === "phone" ? 20 : 254}
        ...
        <Button className="w-full" type="submit" disabled={!!busy}>
          {busy === "verify" ? "验证中…" : "登录 / 注册"}
        </Button>
        <p className="text-muted-foreground text-center text-xs">
          首次验证通过后自动注册。手机号与邮箱分别注册的账号独立；已有邮箱账号请使用邮箱登录。
        </p>
      </form>
    </div>
  );
