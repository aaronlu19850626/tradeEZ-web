...
def invalidate_previous_codes(db: DBConnection, email: str, now: int) -> None:
    db.execute(
        "UPDATE auth_codes SET consumed=1, consumed_at=? WHERE email=? AND consumed=0",
        (now, email),
    )

def latest_code_time(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("SELECT created_at FROM auth_codes WHERE email = ? ORDER BY id DESC LIMIT 1", params)


def code_count_since(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("SELECT COUNT(*) AS count FROM auth_codes WHERE email = ? AND created_at >= ?", params)


def insert_code(db: DBConnection, params: Sequence[object]) -> DBCursor:
...
def find_valid_code(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("""
...
def exhaust_code(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("UPDATE auth_codes SET consumed = 1, attempts = ?, consumed_at = ? WHERE id = ?", params)


def record_failed_attempt(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("UPDATE auth_codes SET attempts = ? WHERE id = ?", params)


def consume_code(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("UPDATE auth_codes SET consumed = 1, consumed_at = ? WHERE id = ?", params)


def find_user_id(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("SELECT id FROM users WHERE email = ?", params)


def insert_user(db: DBConnection, params: Sequence[object]) -> DBCursor:
    identity = params[0]
...
def record_login(db: DBConnection, params: Sequence[object]) -> DBCursor:
    return db.execute("UPDATE users SET last_login_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE email = ?", params)


def get_user_by_email(db: DBConnection, params: Sequence[object]) -> DBCursor:
