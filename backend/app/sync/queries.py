...
def run_detail(run_id: int, page: int, page_size: int, db: sqlite3.Connection, user: sqlite3.Row) -> SyncRunDetailOut:
...
def my_sync_runs(account_id: int | None, run_status: str | None, page: int, page_size: int, db: sqlite3.Connection, user: sqlite3.Row) -> SyncRunPageOut:
...
def my_api_logs(limit: int, mt5_login: int | None, success: bool | None, db: sqlite3.Connection, user: sqlite3.Row) -> list[ApiLogOut]:
