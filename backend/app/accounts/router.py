...
def maintenance_audits(limit: int = Query(20, ge=1, le=100), db: sqlite3.Connection = Depends(get_db), user: sqlite3.Row = Depends(get_current_user)):
    from .maintenance import audits
    return audits(db, user["id"], limit)


@router.get("/api/v1/my/account-key-rotation-audits")
def key_rotation_audits(limit: int = Query(20, ge=1, le=100), db: sqlite3.Connection = Depends(get_db), user: sqlite3.Row = Depends(get_current_user)):
...
def maintenance_preview(account_id: int, db: sqlite3.Connection = Depends(get_db), user: sqlite3.Row = Depends(get_current_user)):
...
def reset_sync(account_id: int, payload: AccountResetIn, db: sqlite3.Connection = Depends(get_db), user: sqlite3.Row = Depends(get_current_user)):
...
def delete_account(account_id: int, payload: AccountMaintenanceIn, db: sqlite3.Connection = Depends(get_db), user: sqlite3.Row = Depends(get_current_user)):
    from .maintenance import execute
