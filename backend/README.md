...
当前版本按 [EA 同步协议](<../docs/40-架构与数据/EA同步协议.md>) 对齐：**UTC 平仓成交时间游标、原始 body HMAC、成交入库与游标推进两阶段提交**。具体约定和验证边界以当前代码、契约测试和该协议为准。

> 数据库和迁移：[数据库与迁移](../docs/40-架构与数据/数据库与迁移.md)
> 运行环境：[本地开发与并行环境](../docs/60-运行与部署/本地开发与并行环境.md)
