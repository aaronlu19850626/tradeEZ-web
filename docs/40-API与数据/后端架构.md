...
版本：历史重构记录；状态：部分过时，需以当前代码复核；用途：追溯服务端模块拆分过程；维护人：后端负责人；验收入口：当前后端测试与模块 PR。

当前运行数据库是 PostgreSQL，迁移头和连接方式以 `backend/app/migrations.py`、`backend/app/db.py` 和《数据库迁移》为准。本文后续出现的 SQLite 连接与锁说明只代表历史实现或兼容层，不能用于设计新代码。

最新进展：已增加独立 Trade 生命周期持久化、反转拆分、成交费用分摊、变更持仓重建和 SQL 分页；迁移 `0002_trade_lifecycles` 已在备份后的实际库执行。54 项测试和 EA 冒烟通过。详见 [交易生命周期](../50-连接器与EA/交易生命周期.md)。

最新覆盖：已按 SOP v1.03 的平仓时间游标和最小请求完成七接口对齐，49 项后端测试与冒烟测试通过。下文记录包含旧阶段历史，当前行为以 [SOP 对齐记录](../50-连接器与EA/SOP契约验收.md) 为准。订单归并已补充 OUT_BY、开仓费用、真实 IN 时间和待核对状态。
...
1. Alembic baseline 已完成：`app/migrations.py` 统一启动和 CLI 迁移，`migrations/versions/0001_sync_baseline.py` 固化旧库兼容升级；操作见 [数据库迁移与恢复](数据库迁移.md)。后续结构变化继续增加 revision。
...
新增 `scripts/local/start-local-preview.ps1`，同时设置网页和 EA 的 API 环境变量，避免本机 HTTP 8000 预览继续显示未运行的 HTTPS 8443 地址。脚本不修改局域网环境文件，不自动终止已有进程。已实际运行并确认网页登录页与后端健康检查响应正常。

账号页提供主 EA 参数模板、WebRequest 白名单和同机访问说明；创建/重置时可复制含完整 Key 的参数。修复重置 Key 后因账号对象刷新而清空新密钥的问题，切换账号时清理密钥状态，异步完成的旧账号操作不重新打开抽屉。操作说明见 [EA 本机接入](../60-运行与部署/EA本地配置.md)。这轮未修改后端协议，未进行真实 MT5 终端联调；也不代表正式 Trade 构建器或公网部署已完成。
