...
基线：tradeEZ.mq5 v1.03，源码头部最后修改时间 2026-09-18 15:25（北京时间）。本文依据 EA 客户端代码整理，未检查服务端实现、未调用线上接口。字段为客户端实际发送字段；服务端必填约束、数据库结构、错误码和认证容差需由后端另行确认。本文不修改 EA。
...
| 入口 | 条件 | 行为 |
|---|---|---|
| 常规定时 | 启用同步、密钥非空、全账户无持仓、未编辑价格、用户操作后空闲至少 5 秒 | 按优先级执行一个到期任务 |
| 手动同步 | 点击“同步数据”，确认阻塞风险 | 允许有持仓，执行完整同步 |
| 每日初始化 | 当前北京时间统计日尚未尝试，且未编辑价格 | 允许有持仓，执行完整同步；首次挂载也可能触发 |

每日标记键为 TEZ.SyncDay.账号.当前品种，统计日起点默认北京时间 04:50。标记在请求之前写入并刷新；失败也算当日已尝试，不自动重跑同一天的完整初始化同步。
...
前一个任务失败不会阻止后续任务执行。最终输出每项 OK/FAIL；品种与配置失败会设置待补发标记。
...
| 1 | 品种规格 | 待发送标记 |
| 2 | 参数配置 | 待发送标记 |
| 3 | 成交同步 | Inp_SyncIntervalMin，默认 5 分钟 |
| 4 | 账户快照 | 累计 30 秒 |
| 5 | 心跳 | 累计 300 秒 |
| 6 | 参数配置 | 累计 3600 秒 |

周期由定时器步长累加，不保证严格墙钟间隔。有持仓时保留累计到期状态，空仓且界面空闲后才执行。待发送标记在补发前清除，该次失败不自动恢复标记。周期任务在请求前清零计数，失败一般等待下一周期。Inp_AlignIntervalHours 虽存在输入和计数，当前没有接入哈希校准接口。

### 2.2 单次请求

1. 非强制同步先再次检查全账户 PositionsTotal()；有仓则返回 -1，不发请求。
2. 显示同步状态、主面板遮罩与提示。
3. 执行同步 WebRequest，保存原始 MT5 错误。
4. 无论成功、失败、超时，都恢复空闲并移除该次请求遮罩。
5. 按上表状态码判断结果，调试模式记录响应。

实际单请求超时参数是 clamp(Inp_RequestTimeoutMS, 200, 800) 毫秒；一轮可能有多次请求，不代表整轮最多 800 毫秒。遮罩按每次请求切换，不是后台异步执行。空仓检查不包含挂单，请求期间仍可能出现挂单成交。
...
仅 HTTP 2xx 判断成功，不读取响应业务字段。建议服务端在成交持久化完成后才允许推进，并采用单调递增更新以防多实例把游标写回旧值；当前 EA 不传版本号、锁或比较条件，无法保证多实例并发安全。
...
| request_timeout_ms | integer | Inp_RequestTimeoutMS | 原始超时输入；实际请求限制为200–800ms |
| max_batch_size | integer | Inp_MaxBatchSize | 原始批量输入；实际发送限制为1–1000 |
| debug | boolean | Inp_DebugSync | 调试日志开关 |

共 55 个 settings 叶子字段。仅 HTTP 2xx 判断成功。密钥不在 settings 中；也没有上传全部 66 个输入，例如语言、重置按钮显示、周计划分档、校准间隔等未全部包含。此接口是 EA 向服务器上传配置，不是拉取远程配置，不会让服务器下发值覆盖本地 EA。

顶层没有 symbol、instance_id、EA 版本或配置 ID；同账号多图表实例如何区分配置快照，需后端补充约定。
...
仅 HTTP 200 成功。正文没有时间、版本、连接状态或品种；请求头有签名时间。后端可记录接收时间作为在线依据，但当前常规心跳受“空仓+界面空闲”限制，持仓期间可能长时间不发送，不能简单用 5 分钟未收到心跳判定 EA 已离线。
