...
本轮 61 项浏览器回归全部通过。自动化覆盖已经能够验证账户导入、三种交易视图、分组明细懒加载、列显示、分页排序、总览评分、累计全屏、交易时段设置、日历复用和 MT5 账户从创建到删除的完整生命周期。后端测试同步保持 96 项通过。`PERF-001` 主体优化已完成，性能和口径对照见《PERF-001 性能优化报告》。
