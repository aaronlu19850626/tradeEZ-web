...
- Key 格式：`prefix.secret`（prefix 明文索引，secret 仅生成时显示一次）。
- 请求头：
  - `Authorization: Bearer <prefix.secret>`
  - `X-Timestamp: <unix秒>`
  - `X-Signature: HMAC-SHA256(body, secret)` (hex)
- 服务端：按 prefix 查 account → 校验 `sha256(secret)==key_hash` → 验签 → 拒收 5 分钟外请求（防重放）→ 校验 `key_revoked=false`。
...
POST /api/v1/ingest/deals
{
  ...
  "deals": [{
    ...
    "magic":920718, "comment":"GoldSOP-TR", "deal_time":1753082193
  }]
}
```
`entry`: 0=IN 1=OUT 2=INOUT · `type`: 0=buy 1=sell · `deal_time`: 服务器 epoch。

响应（幂等，可安全重发）：
```json
{"accepted":1, "duplicated":0, "rejected":0, "server_max_ticket":1010389473}
```
...
否则网络慢会卡下单/平仓。改为"入队 + Timer flush"。

### 3.1 新增输入
```
input bool   Inp_SyncEnable = false;
input string Inp_SyncUrl    = "https://api.yourdomain.com";
input string Inp_SyncKey    = "";        // Web 绑定页生成
```

### 3.2 状态
- `last_synced_ticket` → GlobalVariable（跨重载存活）。
- 待发队列 → 文件 JSONL（未确认成交；发送成功后清除对应行）。

### 3.3 流程
```
OnTradeTransaction(trans):
    if trans.type == DEAL_ADD:
        d = HistoryDealGet(trans.deal)   // 原始字段;IN成交记录sl/tp/magic/comment
        append_to_queue(d)               // 只入队,不发网络

OnTimer():   // 每 1-2 秒,非关键路径
    if not Inp_SyncEnable or queue empty: return
    batch = read_queue(max 50)
    res = WebRequest(POST, url+"/ingest/deals", auth+hmac, json(batch))
    if res.code == 200:
        last_synced_ticket = res.server_max_ticket
        clear_sent_from_queue()
    else: backoff_retry()               // 指数退避,保留队列

    // 另一路:每 30-60 秒发一次快照(余额/净值/保证金)

OnInit(): if 首次绑定:
    HistorySelect(近N天) → 遍历成交入队 → OnTimer 分批补发
```
...
- 注释兜底：magic 未命中时用 comment 前缀（GoldSOP-SC/TR）识别。
...
### 4.5 统计日边界
- 北京时间 `reset_hour:reset_minute`（默认 4:30）翻日，与 EA 一致。
- 服务端存 UTC，用 account 的 `reset_hour/minute` + `server_gmt_off` 换算归日。
