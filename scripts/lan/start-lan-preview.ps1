...
if (!(Get-NetTCPConnection -LocalAddress $address -LocalPort 3000 -State Listen -ErrorAction SilentlyContinue)) {
    Start-Process -FilePath (Get-Command node).Source -ArgumentList @(('"' + (Join-Path $PSScriptRoot 'lan-preview.cjs') + '"')) `
...
& (Join-Path $projectRoot 'backend\scripts\start-lan-api.ps1')
